...
import { cities, macroServices, microServices, testimonials } from '@/data/db';
  ...
  for (const locale of locales) {
    for (const city of cities) {
      for (const micro of microServices) {
        const macro = macroServices.find(m => m.id === micro.macro_id);
        if (macro) {
          params.push({ locale, city: city.slug, macro: macro.slug, micro: micro.slug });
        }
      }
    }
  }
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string, city: string, macro: string, micro: string }> }) {
  const { city: citySlug, macro: macroSlug, micro: microSlug, locale } = await params;
  const city = cities.find(c => c.slug === citySlug);
  const macro = macroServices.find(s => s.slug === macroSlug);
  const micro = microServices.find(s => s.slug === microSlug);
  const dict = await getDictionary(locale);
  
  if (!city || !macro || !micro) return { title: 'Nicht gefunden' };
  
  return {
    title: `${micro.title} in ${city.name} | AZ-Europa Service GmbH`,
    description: micro.description,
  };
}

export default async function MicroServiceMatrixPage({ params }: { params: Promise<{ locale: string, city: string, macro: string, micro: string }> }) {
  const { city: citySlug, macro: macroSlug, micro: microSlug, locale } = await params;
  const city = cities.find(c => c.slug === citySlug);
  const macro = macroServices.find(s => s.slug === macroSlug);
  const micro = microServices.find(s => s.slug === microSlug);
  ...
  const relatedTestimonials = testimonials.filter(t => t.serviceId === macro.id && t.cityId === city.id).slice(0, 3);
  ...
  const localizedServiceTitle = `${micro.title} in ${city.name}`;
  const localizedServiceDesc = micro.description;
  const contentTitle = dict.service_page.content_title.replace('{service}', micro.title);
  const featuresToDisplay = micro.features || dict.service_page.feature_points;
  const ctaTitle = dict.service_page.offer_card_title.replace('{service}', micro.title);

  return (
    <div className={`flex flex-col min-h-screen bg-[#f7f9fb] overflow-x-hidden ${isRTL ? 'text-right' : 'text-left'}`}>
      <LocalBusinessSchema city={city} service={{ id: micro.id, title: micro.title, description: micro.description }} />
      <ServiceSchema 
        serviceName={micro.title}
        description={micro.description}
      ...
      <CinematicHeroParallax
        ...
        backgroundImage={micro.image || macro.cover_image}
        ...
        breadcrumbs={
          <BreadcrumbSchema 
            ...
            items={[
              ...
              { label: micro.title, href: getLocalizedHref(`/${city.slug}/${macro.slug}/${micro.slug}`) }
            ]} 
          />
        }
      />
