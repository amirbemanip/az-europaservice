...
export function InteractiveRegionMap({ locale, dict }: { locale: string; dict: any }) {
  ...
  const localizedTitle = dict.region_map.title_template.replace(
    '{region}',
    `<span class="text-[#d97706]">${dict.region_map.region_highlight}</span>`
  );
  ...
  return (
    <section className={`py-32 bg-[#fafafa] relative overflow-hidden z-20 border-t border-black/5 ${isRTL ? 'text-right' : 'text-left'}`}>
      ...
      {/* Map Glow */}
      <div className={`absolute top-1/2 ${isRTL ? 'left-0' : 'right-0'} w-[500px] h-[500px] bg-[#d97706]/8 rounded-full blur-[120px] -translate-y-1/2 pointer-events-none`} />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className={`flex flex-col lg:flex-row gap-16 items-center ${isRTL ? 'lg:flex-row-reverse' : ''}`}>
          ...
          >
            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#d97706]/10 border border-[#fed01b]/20 text-[#d97706] text-sm font-bold tracking-widest uppercase mb-6 shadow-sm ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className="w-16 h-16 rounded-full bg-white flex items-center justify-center border border-black/5 shadow-md">
                <Navigation className="w-6 h-6 text-[#d97706]" />
              </div>
              ...
              {cities.map((city, i) => (
                ...
                >
                  <Link href={getLocalizedHref(`/${city.slug}`)} className="block group">
                    <div className="bg-white p-8 rounded-3xl h-full flex flex-col relative overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-black/5 hover:border-[#d97706]/50 group">
                      ...
                      <div className={`absolute top-8 ${isRTL ? 'left-8' : 'right-8'}`}>
                        <div className="relative flex h-4 w-4">
                          ...
                          <span className="relative inline-flex rounded-full h-4 w-4 bg-[#d97706]/80 border border-[#fed01b]"></span>
                        </div>
                      </div>
  
                      <div className={`w-12 h-12 bg-slate-50 border border-black/5 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-[#d97706] group-hover:scale-110 transition-all duration-300 ${isRTL ? 'mr-auto' : ''}`}>
                        <MapPin className="w-6 h-6 text-[#d97706] group-hover:text-[#0a0a0a] transition-colors" />
                      </div>
                      ...
                      <div className={`pt-6 border-t border-black/5 flex justify-between items-center mt-auto ${isRTL ? 'flex-row-reverse' : ''}`}>
                        ...
                        <div className="w-10 h-10 rounded-full bg-slate-50 border border-black/5 flex items-center justify-center group-hover:bg-[#d97706]/20 transition-colors">
